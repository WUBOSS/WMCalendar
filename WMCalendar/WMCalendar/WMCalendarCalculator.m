...
{
    return [self indexPathForDate:date atMonthPosition:WMCalendarMonthPositionCurrent scope:self.calendar.transitionCoordinator.representingScope];
}
- (NSIndexPath *)indexPathForDate:(NSDate *)date scope:(WMCalendarScope)scope
{
    return [self indexPathForDate:date atMonthPosition:WMCalendarMonthPositionCurrent scope:scope];
}
...
- (NSIndexPath *)indexPathForDate:(NSDate *)date atMonthPosition:(WMCalendarMonthPosition)position scope:(WMCalendarScope)scope
{
    ...
    switch (scope) {
        case WMCalendarScopeMonth: {
            ...
            if (position == WMCalendarMonthPositionPrevious) {
                section++;
            } else if (position == WMCalendarMonthPositionNext) {
                section--;
            }
...
{
    if (!month) return 0;
    if (self.calendar.placeholderType == WMCalendarPlaceholderTypeFillSixRows) return 6;
...
{
    if (self.calendar.transitionCoordinator.representingScope == WMCalendarScopeWeek) return 1;
...
{
    if (!indexPath) return WMCalendarMonthPositionNotFound;
    if (self.calendar.transitionCoordinator.representingScope == WMCalendarScopeWeek) {
        return WMCalendarMonthPositionCurrent;
    }
    ...
    switch (comparison) {
        case NSOrderedAscending:
            return WMCalendarMonthPositionPrevious;
        case NSOrderedSame:
            return WMCalendarMonthPositionCurrent;
        case NSOrderedDescending:
            return WMCalendarMonthPositionNext;
    }
}

- (WMCalendarCoordinate)coordinateForIndexPath:(NSIndexPath *)indexPath
{
    WMCalendarCoordinate coordinate;

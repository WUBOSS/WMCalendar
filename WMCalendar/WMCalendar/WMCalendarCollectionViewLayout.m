//
//  WMCalendarCollectionViewLayout.m
//  WMCalendar
//
...
#define kWMCalendarSeparatorInterRows @"WMCalendarSeparatorInterRows"
#define kWMCalendarSeparatorInterColumns @"WMCalendarSeparatorInterColumns"

@interface WMCalendarCollectionViewLayout ()
...
@implementation WMCalendarCollectionViewLayout
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [self registerClass:[WMCalendarSeparator class] forDecorationViewOfKind:kWMCalendarSeparatorInterRows];
    }
    return self;
}
...
- (void)prepareLayout
{
    ...
    self.estimatedItemSize = ({
        ...
        CGFloat height = ({
            CGFloat height = WMCalendarStandardRowHeight;
            if (!self.calendar.floatingMode) {
    ...
    self.widths = ({
        ...
        WMCalendarSliceCake(contentWidth, columnCount, widths);
        widths;
    });
    ...
    free(self.heights);
    self.heights = ({
        NSInteger rowCount = self.calendar.transitionCoordinator.representingScope == WMCalendarScopeWeek ? 1 : 6;
        ...
        if (!self.calendar.floatingMode) {
            ...
            WMCalendarSliceCake(contentHeight, rowCount, heights);
        } else {
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if (!self.calendar.floatingMode) {
        ...
        switch (self.scrollDirection) {
            case UICollectionViewScrollDirectionHorizontal: {
                ...
                NSInteger startColumn = ({
                    ...
                    CGFloat widthDelta = WMCalendarMod(CGRectGetMinX(rect), self.collectionView.fs_width)-self.sectionInsets.left;
                    widthDelta = MIN(MAX(0, widthDelta), self.collectionView.fs_width-self.sectionInsets.left);
                    NSInteger countDelta = WMCalendarFloor(widthDelta/self.estimatedItemSize.width);
                ...
                NSInteger endColumn = ({
                    ...
                    CGFloat remainder = WMCalendarMod(section, 1);
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endColumn = WMCalendarFloor(section)*7 - 1;
                    } else {
                        CGFloat widthDelta = WMCalendarMod(CGRectGetMaxX(rect), self.collectionView.fs_width)-self.sectionInsets.left;
                        widthDelta = MIN(MAX(0, widthDelta), self.collectionView.fs_width - self.sectionInsets.left);
                        NSInteger countDelta = WMCalendarCeil(widthDelta/self.estimatedItemSize.width);
                        endColumn = WMCalendarFloor(section)*7 + countDelta - 1;
                    }
                    endColumn;
                });
                
                NSInteger numberOfRows = self.calendar.transitionCoordinator.representingScope == WMCalendarScopeMonth ? 6 : 1;
                ...
                for (NSInteger column = startColumn; column <= endColumn; column++) {
                    for (NSInteger row = 0; row < numberOfRows; row++) {
                        ...
                        UICollectionViewLayoutAttributes *rowSeparatorAttributes = [self layoutAttributesForDecorationViewOfKind:kWMCalendarSeparatorInterRows atIndexPath:indexPath];
            ...
            case UICollectionViewScrollDirectionVertical: {
                ...
                NSInteger startRow = ({
                    ...
                    CGFloat heightDelta = WMCalendarMod(CGRectGetMinY(rect), self.collectionView.fs_height)-self.sectionInsets.top;
                    heightDelta = MIN(MAX(0, heightDelta), self.collectionView.fs_height-self.sectionInsets.top);
                    NSInteger countDelta = WMCalendarFloor(heightDelta/self.estimatedItemSize.height);
                ...
                NSInteger endRow = ({
                    ...
                    // https://stackoverflow.com/a/10335601/2398107
                    if (remainder <= MAX(100*FLT_EPSILON*ABS(remainder), FLT_MIN)) {
                        endRow = WMCalendarFloor(section)*6 - 1;
                    } else {
                        CGFloat heightDelta = WMCalendarMod(CGRectGetMaxY(rect), self.collectionView.fs_height)-self.sectionInsets.top;
                        heightDelta = MIN(MAX(0, heightDelta), self.collectionView.fs_height-self.sectionInsets.top);
                        NSInteger countDelta = WMCalendarCeil(heightDelta/self.estimatedItemSize.height);
                        endRow = WMCalendarFloor(section)*6 + countDelta-1;
                    }
                    endRow;
                });
        ...
        NSInteger startRowIndex = ({
            ...
            NSInteger startRowCount = WMCalendarCeil(heightDelta1/self.estimatedItemSize.height);
        ...
        NSInteger endRowIndex = ({
            ...
            NSInteger endRowCount = WMCalendarCeil(heightDelta2/self.estimatedItemSize.height);
...
{
    WMCalendarCoordinate coordinate = [self.calendar.calculator coordinateForIndexPath:indexPath];
...
{
    if ([elementKind isEqualToString:kWMCalendarSeparatorInterRows] && (self.separators & WMCalendarSeparatorInterRows)) {
        ...
        if (!attributes) {
            ...
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForDecorationViewOfKind:kWMCalendarSeparatorInterRows withIndexPath:indexPath];
            CGFloat x, y;
            if (!self.calendar.floatingMode) {
                switch (self.scrollDirection) {
                    case UICollectionViewScrollDirectionHorizontal: {
            ...
            CGFloat width = self.collectionView.fs_width;
            CGFloat height = WMCalendarStandardSeparatorThickness;
...
#undef kWMCalendarSeparatorInterColumns
#undef kWMCalendarSeparatorInterRows

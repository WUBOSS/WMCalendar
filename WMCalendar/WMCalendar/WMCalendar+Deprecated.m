//
//  WMCalendar+Deprecated.m
//  WMCalendar
//
//  Created by zhanshu1 on 2017/8/18.
...
@implementation WMCalendar (Deprecated)

@dynamic identifier, lineHeightMultiplier;

- (void)setShowsPlaceholders:(BOOL)showsPlaceholders
{
    self.placeholderType = showsPlaceholders ? WMCalendarPlaceholderTypeFillSixRows : WMCalendarPlaceholderTypeNone;
}

- (BOOL)showsPlaceholders
{
    return self.placeholderType == WMCalendarPlaceholderTypeFillSixRows;
}
...
- (NSDate *)tomorrowOfDate:(NSDate *)date
{
    ...
    components.day++;
    components.hour = WMCalendarDefaultHourComponent;
    return [self.gregorian dateFromComponents:components];
}
...
{
    return [self isDate:date equalToDate:[NSDate date] toCalendarUnit:WMCalendarUnitDay];
}
...
{
    self.rowHeight = WMCalendarStandardRowHeight*MAX(1, WMCalendarDeviceIsIPad*1.5);
}
